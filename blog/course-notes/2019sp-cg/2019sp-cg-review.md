...
1. **直线生成DDA算法**
	...
	- 分类讨论
		- $|m|\le1$：以$\Delta x=\pm1$取样，计算$y_{k+1}=y_k\pm m$.
		- $|m|>1$：以$\Delta y=\pm1$取样，计算$x_{k+1}=x_k\pm\frac{1}{m}$.
	- 特性
...
2. **区域填充图元生成**
	...
	- **像素连通定义**
		+ **像素4连通**：定义城区距离$d_4(p,r)=|x_p-x_r|+|y_p-y_r|$. 则像素$p(x_p,y_p)$的4连通区域或4邻域$N_4(p)$定义为$N_4(p)=\{r|d_4(p,r)=1\}$.
		+ **像素8连通**：定义棋盘距离$d_8(p,r)=\mathrm{max}(|x_p-x_r|,|y_p-y_r|)$. 则像素$p(x_p,y_p)$的8邻域$N_8(p)$定义为$N_8(p)=\{r|d_8(p,r)=1\}$.
		+ **像素16连通**：定义马步距离$d_k(p,r)$为国际象棋马从一格运动到另一格所需步数计算，$N_k(p)=\{r|d_k(p,r)=1\}$. 定义像素$p(x_p,y_p)$的16邻域$N_{16}(p)$为$N_{16}(p)=N_8(p)\cup N_k(p)$.
...
1. **三次插值样条曲线**
	...
	- 类型：按基底划分，有分段/整体表示形式；按所取数据点的参数化方法划分，有均匀参数三次样条曲线（$\Delta u$为常数）、积累弦长参数三次样条曲线（$u_0=0, u_i=u_{i-1}+|\Delta p_{i-1}|$）、向心参数三次样条曲线（$u_0=0,u_i=u_{i-1}+|\Delta p_{i-1}|^{1/2}$）、修正弦长参数三次样条曲线$u_0=0,u_i=u_{i-1}+k_i|\Delta p_{i-1}|,k_i\ge1$）
2. **三次插值样条生成问题**
...
2. **B样条曲线定义**
	- 给定$n+1$个控制顶点$\{P_i\}(i=0,1,\dots,n)$，和$n+k+2$个参数节点向量$U_{n,k}=\{u_i|i=0,1,\dots,n+k+1,u_i\le u_{i+1}\}$.
...
12. **三次均匀B样条曲线**
	- 令$\Delta>0,u_0=0$且节点向量为$U_{n,k}=\{u_i|u_i=i\}(i=0,1,2,\dots,n+k)$. $U_{n,k}$上的B样条基函数为$$N_{i,k}(u)=\frac{u-i}{k-1}N_{i,k-1}(u)+\frac{i+k-u}{k-1}N_{i+1,k-1}(u),k>1$$. 当$u\in[i,i+1)$时有$N_{i,1}(t)=1$，否则$N_{i,1}(t)=0$.
