...
- $$SampleRTT$$与$$EstimatedRTT$$之间差值的EWMA：$$DevRTT = (1-\beta)\cdot DevRTT + \beta\cdot\mid SampleRTT - EstimatedRTT\mid$$
...
- 算法比较
	+ **报文复杂性**：LS算法要发送$$O(\mid N\mid\mid E\mid)$$个报文，无论何时一条链路的费用发生改变，都要向所有结点发送新的链路费用。DV算法在每次迭代时仅在两个相邻邻居之间交换报文，且仅当在新的链路费用导致与该链路相连结点的最低费用路径发生改变时才传播已改变的链路费用。
	+ **收敛速度**：LS算法朴素实现是一个$$O(\mid N\mid ^2)$$的算法，而DV算法收敛较慢而且可能遭遇无穷计数问题。
